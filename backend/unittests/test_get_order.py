...
class TestGetOrderFromDDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from handlers.get_order.get_order import get_order_from_ddb
        from handlers.get_order.get_order import dynamodb

        self.get_order_from_ddb = get_order_from_ddb  # Store it in an instance variable
    ...
    @mock_aws
    def test_get_order_from_ddb_success(self):
        print(f'***************************************************')
        ...
        get_order_from_ddb = self.get_order_from_ddb
        ...
        # Call the function under test to get the order whose id is '1'
        result = get_order_from_ddb(table_name, '1', valerror)

        # Check the result
        self.assertIsNotNone(result)
        self.assertEqual(len(result), 7)
        self.assertEqual(result['id'], '1')
        self.assertEqual(result['customer_name'], 'Customer1')
        self.assertEqual(valerror, {'error':''})  # No error should be set

    @mock_aws
    def test_get_order_from_ddb_table_not_found(self):
        print(f'***************************************************')
        print(f'Unit Test: {self.__class__.__name__} : {self._testMethodName} :')
        print(f'***************************************************')    
